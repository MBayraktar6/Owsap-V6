...
{
	DWORD dwCurMotionIndex = __GetCurrentMotionIndex();
	if (dwCurMotionIndex > CRaceMotionData::NAME_SKILL_END && dwCurMotionIndex < CRaceMotionData::NAME_MAX_NUM)
		return TRUE;
...
bool CActorInstance::__CanPushDestActor(CActorInstance& rkActorDst)
{
	...
	{
		if (rkActorDst.GetRank() == NRaceData::MOB_RANK_BOSS)
			return false;

		if (rkActorDst.GetRank() == NRaceData::MOB_RANK_KING)
			return false;
	}
#endif
...
#if defined(__DISABLE_BOSS_PUSHBACK__)
		if (rVictim.GetRank() != NRaceData::MOB_RANK_BOSS && rVictim.GetRank() != NRaceData::MOB_RANK_KING)
...
void CActorInstance::__HitGood(CActorInstance& rVictim)
{
	...
	if (rVictim.IsEnemy())
	{
		if (rVictim.GetRank() == NRaceData::MOB_RANK_BOSS)
			return;

		if (rVictim.GetRank() == NRaceData::MOB_RANK_KING)
			return;
	}
#endif
