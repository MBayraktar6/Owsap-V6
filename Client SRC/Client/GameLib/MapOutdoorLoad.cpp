...
bool CMapOutdoor::LoadTerrain(WORD wTerrainCoordX, WORD wTerrainCoordY, WORD wCellCoordX, WORD wCellCoordY)
{
	...
	char szMiniMapLeftTopTextName[64 + 1];
	char szMiniMapTopTexName[64 + 1];
	char szMiniMapRightTopTexName[64 + 1];
	char szMiniMapLeftTexName[64 + 1];
	char szMiniMapCenterTexName[64 + 1];
	char szMiniMapRightTexName[64 + 1];
	char szMiniMapLeftBottomTexName[64 + 1];
	char szMiniMapBottomTexName[64 + 1];
	char szMiniMapRightBottomTexName[64 + 1];
#else
	...
	// NOTE : The only thing we want to change in a copied map at the moment will be the attributes.
	if (IsCopiedMap())
	{
		_snprintf(szAttrMapName, sizeof(szAttrMapName), "%s\\%06u\\attr.atr", GetName().c_str(), ulID);

		CMappedFile	kMappedFile; LPCVOID lpcvFileData;
		if (!CEterPackManager::Instance().Get(kMappedFile, szAttrMapName, &lpcvFileData))
...
#if defined(ENABLE_MINIMAP_TEXTURE_PATTERN)
	_snprintf(szMiniMapLeftTopTextName, sizeof(szMiniMapLeftTopTextName), "%s\\%06u\\minimap_lefttop.dds", GetMapDataDirectory().c_str(), ulID);
	_snprintf(szMiniMapTopTexName, sizeof(szMiniMapTopTexName), "%s\\%06u\\minimap_top.dds", GetMapDataDirectory().c_str(), ulID);
	_snprintf(szMiniMapRightTopTexName, sizeof(szMiniMapRightTopTexName), "%s\\%06u\\minimap_righttop.dds", GetMapDataDirectory().c_str(), ulID);
	_snprintf(szMiniMapLeftTexName, sizeof(szMiniMapLeftTexName), "%s\\%06u\\minimap_left.dds", GetMapDataDirectory().c_str(), ulID);
	_snprintf(szMiniMapCenterTexName, sizeof(szMiniMapCenterTexName), "%s\\%06u\\minimap.dds", GetMapDataDirectory().c_str(), ulID);
	_snprintf(szMiniMapRightTexName, sizeof(szMiniMapRightTexName), "%s\\%06u\\minimap_right.dds", GetMapDataDirectory().c_str(), ulID);
	_snprintf(szMiniMapLeftBottomTexName, sizeof(szMiniMapLeftBottomTexName), "%s\\%06u\\minimap_leftbottom.dds", GetMapDataDirectory().c_str(), ulID);
	_snprintf(szMiniMapBottomTexName, sizeof(szMiniMapBottomTexName), "%s\\%06u\\minimap_bottom.dds", GetMapDataDirectory().c_str(), ulID);
	_snprintf(szMiniMapRightBottomTexName, sizeof(szMiniMapRightBottomTexName), "%s\\%06u\\minimap_rightbottom.dds", GetMapDataDirectory().c_str(), ulID);
#else
...
#if defined(ENABLE_MINIMAP_TEXTURE_PATTERN)
	pTerrain->LoadMiniMapTexture(szMiniMapLeftTopTextName, CTerrain::MINIMAP_TEXTURE_LEFT_TOP);
	pTerrain->LoadMiniMapTexture(szMiniMapTopTexName, CTerrain::MINIMAP_TEXTURE_TOP);
	pTerrain->LoadMiniMapTexture(szMiniMapRightTopTexName, CTerrain::MINIMAP_TEXTURE_RIGHT_TOP);
	pTerrain->LoadMiniMapTexture(szMiniMapLeftTexName, CTerrain::MINIMAP_TEXTURE_LEFT);
	pTerrain->LoadMiniMapTexture(szMiniMapCenterTexName, CTerrain::MINIMAP_TEXTURE_CENTER);
	pTerrain->LoadMiniMapTexture(szMiniMapRightTexName, CTerrain::MINIMAP_TEXTURE_RIGHT);
	pTerrain->LoadMiniMapTexture(szMiniMapLeftBottomTexName, CTerrain::MINIMAP_TEXTURE_LEFT_BOTTOM);
	pTerrain->LoadMiniMapTexture(szMiniMapBottomTexName, CTerrain::MINIMAP_TEXTURE_BOTTOM);
	pTerrain->LoadMiniMapTexture(szMiniMapRightBottomTexName, CTerrain::MINIMAP_TEXTURE_RIGHT_BOTTOM);
#else
