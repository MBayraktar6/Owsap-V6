...
bool CRaceData::GetFlyTargetAdjustPosition(D3DXVECTOR3* v3Position) const
{
	if (!m_bHasFlyTargetAdjustPosition)
		return false;

	*v3Position = m_v3FlyTargetAdjustPosition;
	return true;
}
...
void CRaceData::__Initialize()
{
	...
	m_bHasFlyTargetAdjustPosition = false;
	m_v3FlyTargetAdjustPosition = { 0.0f, 0.0f, 0.0f };
}

void CRaceData::LoadAllMotions()
{
	//TMotionModeDataIterator it = m_pMotionModeDataMap.begin();
	//for (it; it != m_pMotionModeDataMap.end(); ++it)
	//{
	//	TMotionModeData* const& pMotionModeData = it->second;
	//	if (pMotionModeData == nullptr)
	//		continue;
	//
	//	for (const TMotionVectorMap::value_type motion_map : pMotionModeData->MotionVectorMap)
	//	{
	//		for (const TMotionVector::const_iterator::value_type& motion : motion.second)
	//		{
