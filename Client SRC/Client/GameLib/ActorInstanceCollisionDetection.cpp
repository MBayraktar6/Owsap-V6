	...
	for (itor = m_DefendingPointInstanceList.begin(); itor != m_DefendingPointInstanceList.end(); ++itor)
	{
		...
		for (DWORD i = 0; i < c_pAttackingSphereVector->size(); ++i)
		{
			for (DWORD j = 0; j < c_pDefendingSphereVector->size(); ++j)
			{
				...
				{
#if defined(__HIT_RANGE_RENEWAL__)
					const float c_fHitRange = CPythonNonPlayer::Instance().GetMonsterHitRange(GetRace());
					const D3DXVECTOR3 c_v3DestPos = c_rAttackingSphere.v3Position;
	
					D3DXVECTOR3 v3SelfPos = GetPosition();
					v3SelfPos.z += (GetScale().z - 1.0f) * (c_fHitRange / GetScale().z);

					const D3DXVECTOR3 c_v3Distance = c_v3DestPos - v3SelfPos;

					D3DXVECTOR3 v3NormalizeDistance;
					D3DXVec3Normalize(&v3NormalizeDistance, &c_v3Distance);

					v3NormalizeDistance *= c_fHitRange;

					D3DXVECTOR3 v3NewPos = v3SelfPos + v3NormalizeDistance * 0.5f;
					*pv3Position = v3NewPos;
#else
...
BOOL CActorInstance::__SplashAttackProcess(CActorInstance& rVictim)
{
	...
	float fHitRange = CPythonNonPlayer::Instance().GetMonsterHitRange(GetRace());

	float fScale = CGraphicObjectInstance::GetScale().x;
	float fRange = (fScale - 1.0f) * fHitRange + 1000.0f;

	if ((fRange * fRange) <= fDistance)
		return FALSE;
#else
	D3DXVECTOR3 v3Distance(rVictim.m_x - m_x, rVictim.m_z - m_z, rVictim.m_z - m_z);
	float fDistance = D3DXVec3LengthSq(&v3Distance);
	...
	if (rVictim.CheckCollisionDetection(&m_kSplashArea.SphereInstanceVector, &v3HitPosition))
	{
		...
		int iMaxHitCount = (0 == c_rAttackData.iHitLimitCount ? NRaceData::MAX_HIT_COUNT : c_rAttackData.iHitLimitCount);
		//Tracef(" ------------------- Splash Hit : %d\n", iCurrentHitCount);
...
#if defined(__HIT_RANGE_RENEWAL__)
	float fMyHitRange = CPythonNonPlayer::Instance().GetMonsterHitRange(GetRace());
	float fMonsterHitRange = CPythonNonPlayer::Instance().GetMonsterHitRange(rVictim.GetRace());

	float fMaxDistance = fMonsterHitRange + fMyHitRange + 130.0f;
	if (fDistance >= fMaxDistance * fMaxDistance)
		return FALSE;
#else
...
#if defined(__HIT_RANGE_RENEWAL__)
	const float c_fAttackRadius = GetScale().x * 20.0f;
#else
				...
				for (dsit = c_DefendingSphereVector.begin(); dsit != c_DefendingSphereVector.end(); ++dsit, ++index)
				{
					...
					if (DetectCollisionDynamicZCylinderVSDynamicZCylinder(dsi, sub))
					{
						...
						if (itHitData == m_HitDataMap.end())
						{
							...
							HittedInstanceMap.emplace(std::make_pair(&rVictim, GetLocalTime() + pad->fInvisibleTime));
							//HittedInstanceMap.insert(make_pair(&rVictim, GetLocalTime()+HIT_COOL_TIME));
							m_HitDataMap.emplace(std::make_pair(&c_rHitData, HittedInstanceMap));

							//Tracef(" ----------- First Hit\n");
						}
						else
						{
							itHitData->second.emplace(std::make_pair(&rVictim, GetLocalTime() + pad->fInvisibleTime));
							...
							{
								if (iCurrentHitCount > NRaceData::MAX_HIT_COUNT)
								{
...
#if defined(__HIT_RANGE_RENEWAL__)
						D3DXVECTOR3 v3SelfPos = GetPosition();
						float fSelfScale = GetScale().x;
						float fSelfHitPos = fMyHitRange / c_fAttackRadius * (c_fAttackRadius - 1.0f) + v3SelfPos.z;

						D3DXVECTOR3 v3VictimPos = rVictim.GetPosition();
						float fVictimScale = rVictim.GetScale().x;
						float fVictimHitPos = (fVictimScale - 1.0f) * (fMonsterHitRange / fVictimScale) + v3VictimPos.z;

						const D3DXVECTOR3 v3Distance(
							v3SelfPos.x - v3VictimPos.x,
							v3SelfPos.y - v3VictimPos.y,
							v3SelfPos.z - v3VictimPos.z
						);

						D3DXVECTOR3 v3HitPosition;
						D3DXVec3Normalize(&v3HitPosition, &v3Distance);
						v3HitPosition *= fMonsterHitRange;
						v3HitPosition *= 0.5f;
						
						v3HitPosition.x += v3VictimPos.x;
						v3HitPosition.y += v3VictimPos.y;
						v3HitPosition.z += v3VictimPos.z;
#else
...
#if defined(REMOVE_HORSE_COLLISION)
	if (rVictim.IsHorse())
		return FALSE;
#endif

#if defined(REMOVE_PET_COLLISION)
	if (rVictim.IsPet() || rVictim.IsPetPay())
		return FALSE;
#endif

#if defined(REMOVE_SAFEZONE_COLLISION)
	if (IsInSafeZone(rVictim))
		return false;
#endif

#if defined(REMOVE_SHOP_COLLISION)
...
#if defined(REMOVE_OX_PLAYER_COLLISION)
	if ("metin2_map_oxevent" == CPythonBackground::Instance().GetWarpMapName())
	{
...
#if defined(REMOVE_BAZAR_COLLISION)
	if ("metin2_map_privateshop" == CPythonBackground::Instance().GetWarpMapName())
		return false;
#endif
