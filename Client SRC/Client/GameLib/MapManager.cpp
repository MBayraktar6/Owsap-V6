...
bool CMapManager::LoadMap(const std::string& c_rstrMapName, float x, float y, float z)
{
	...
	if (!rkMap.LoadProperty() || !rkMap.LoadSungMaAttr())
		return false;
#else
	rkMap.LoadProperty();
#endif
	...
	else if (CMapBase::MAPTYPE_OUTDOOR == rkMap.GetType())
	{
		...
		RegisterEnvironmentData(CPythonBackground::DAY_MODE_LIGHT, rkMap.GetEnvironmentDataName().c_str());
		SetEnvironmentData(CPythonBackground::DAY_MODE_LIGHT);
	}
	else
	{
