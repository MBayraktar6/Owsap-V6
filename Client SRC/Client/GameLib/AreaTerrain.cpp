...
void CTerrain::LoadMiniMapTexture(const char* c_pchMiniMapFileName, BYTE bTexturePattern)
{
	if (bTexturePattern >= CTerrain::MINIMAP_TEXTURE_MAX_NUM)
		return;
	...
	m_MiniMapGraphicImageInstance[bTexturePattern].SetImagePointer(pImage);
	if (!m_MiniMapGraphicImageInstance[bTexturePattern].GetTexturePointer()->IsEmpty())
	{
		m_lpMiniMapTexture[bTexturePattern] = m_MiniMapGraphicImageInstance[bTexturePattern].GetTexturePointer()->GetD3DTexture();
	}
	else
	{
		Tracef(" CTerrain::LoadMiniMapTexture - MiniMapTexture Error");
		m_lpMiniMapTexture[bTexturePattern] = nullptr;
	}
}

LPDIRECT3DTEXTURE8 CTerrain::GetMiniMapTexture(BYTE bTexturePattern) const
{
	return m_lpMiniMapTexture[bTexturePattern];
}
#else
