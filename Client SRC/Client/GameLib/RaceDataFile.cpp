...
BOOL CRaceData::LoadRaceData(const char* c_szFileName)
{
	...
	if (TextFileLoader.SetChildNode("flytargetadjustposition"))
	{
		if (!TextFileLoader.GetTokenVector3("adjustposition", &m_v3FlyTargetAdjustPosition))
			return FALSE;

		m_bHasFlyTargetAdjustPosition = true;
		TextFileLoader.SetParentNode();
	}
