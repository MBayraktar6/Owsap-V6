...
void CActorInstance::ProcessMotionEventEffectEvent(const CRaceMotionData::TMotionEventData* c_pData)
{
	...
	if (!m_bRenderActor || !m_bRenderEffect)
		return;
#endif
	...
	{
		int iIndex = CEffectManager::Instance().CreateEffect(c_pEffectData->dwEffectIndex, D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), c_v3Scale.x);
...
void CActorInstance::ProcessMotionEventSpecialAttacking(int iMotionEventIndex, const CRaceMotionData::TMotionEventData* c_pData)
{
	...
	for (DWORD i = 0; i < c_pAttackingData->CollisionData.SphereDataVector.size(); ++i)
	{
		...
		rSphereInstance.v3Position.x = m_x + (c_rSphereData.v3Position.x * fScale) * sinf(fRadian) + (c_rSphereData.v3Position.y * fScale) * cosf(fRadian);
		rSphereInstance.v3Position.y = m_y + (c_rSphereData.v3Position.x * fScale) * cosf(fRadian) - (c_rSphereData.v3Position.y * fScale) * sinf(fRadian);
