...
bool CEterPack::EncryptIndexFile()
{
	...
	CLZObject zObj;
	if (!CLZO::Instance().CompressEncryptedMemory(zObj, pbData, file.Size(), ETERPACK_KEY))
	{
...
bool CEterPack::__BuildIndex(CEterFileDict& rkFileDict, bool bOverwrite)
{
	...
	else if (fourcc == CLZObject::ms_dwFourCC)
	{
		...
		if (!CLZO::Instance().Decompress(zObj, (const BYTE*)pvData, ETERPACK_KEY))
			return false;
...
bool CEterPack::Get(CMappedFile& out_file, const char* filename, LPCVOID* data)
{
	...
	return index && Get2(out_file, filename, index, data) != 0;
}
...
bool CEterPack::Get2(CMappedFile& out_file, const char* filename, TEterPackIndex* index, LPCVOID* data)
{
	...
	*/
	...
	bool bIsSecurityCheckRequired = (index->compressed_type == COMPRESSED_TYPE_SECURITY || index->compressed_type == COMPRESSED_TYPE_PANAMA);
	...
	else if (COMPRESSED_TYPE_SECURITY == index->compressed_type)
	{
		...
		if (!CLZO::Instance().Decompress(*zObj, static_cast<const BYTE*>(*data), ETERPACK_SECURITY_KEY))
		{
...
bool CEterPack::Extract()
{
	...
	for (TDataPositionMap::iterator i = m_DataPositionMap.begin(); i != m_DataPositionMap.end(); ++i)
	{
		...
		{
			if (!CLZO::Instance().Decompress(zObj, (const BYTE*)data + index->data_position, ETERPACK_SECURITY_KEY))
			{
				printf("cannot decompress");
				return false;
			}
...
bool CEterPack::Put(const char* filename, LPCVOID data, long len, BYTE packType)
{
	...
	if (packType == COMPRESSED_TYPE_SECURITY || packType == COMPRESSED_TYPE_COMPRESS)
	{
		...
		{
			if (!CLZO::Instance().CompressEncryptedMemory(zObj, data, len, ETERPACK_SECURITY_KEY))
				return false;
		}
		else
		{
