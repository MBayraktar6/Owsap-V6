...
bool CTextFileLoader::LoadGroup(TGroupNode* pGroupNode)
{
	...
	{
		if (!m_textFileLoader.SplitLine(m_dwcurLineIndex, &stTokenVector))
			continue;
		...
		*/
		...
		else
		{
			...
			pGroupNode->m_TokenVectorNew.push_back(stTokenVector);
...
BOOL CTextFileLoader::GetTokenVectorNew(std::vector<CTokenVector>** ppTokenVector)
{
	if (!m_pcurNode)
		return false;

	*ppTokenVector = &m_pcurNode->m_TokenVectorNew;
	return TRUE;
}
