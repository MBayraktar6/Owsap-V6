...
constexpr float LODHEIGHT_ACTOR = 500.0f;
constexpr float LODDISTANCE_ACTOR = 5000.0f;
constexpr float LODDISTANCE_BUILDING = 25000.0f;

constexpr float c_fNearLodScale = 3.0f;
constexpr float c_fFarLodScale = 25.0f;
constexpr float LOD_APPLY_MAX = 2000.0f;
constexpr float LOD_APPLY_MIN = 500.0f;
...
void CGrannyLODController::Clear()
{
	...
	for (TAttachingModelData& rData : m_AttachedModelDataVector)
...
void CGrannyLODController::AttachModelInstance(CGrannyLODController* pSrcLODController, const char* c_szBoneName)
{
	...
	// Link Child Data
	AttachedModelDataVector::iterator itor = m_AttachedModelDataVector.begin();
...
void CGrannyLODController::SetLODLevel(BYTE bLodLevel)
{
	...
	if (!m_que_pkModelInst.empty())
		m_bLODLevel = static_cast<BYTE>(MIN(m_que_pkModelInst.size() - 1, bLodLevel));
}

void CGrannyLODController::CreateDeviceObjects()
{
	std::for_each(m_que_pkModelInst.begin(), m_que_pkModelInst.end(),
		[&](CGrannyModelInstance* pModelInstance) { if (pModelInstance) pModelInstance->CreateDeviceObjects(); });
}

void CGrannyLODController::DestroyDeviceObjects()
{
	std::for_each(m_que_pkModelInst.begin(), m_que_pkModelInst.end(),
		[&](CGrannyModelInstance* pModelInstance) { if (pModelInstance) pModelInstance->DestroyDeviceObjects(); });
}
...
void CGrannyLODController::UpdateLODLevel(float fDistanceFromCenter, float fDistanceFromCamera)
{
	...
	if (fDistanceFromCenter > LOD_APPLY_MIN) // 중심 LOD 예외 취소
	{
		...
		if (m_fLODDistance > 0.0f)
			m_dwLODAniFPS = static_cast<DWORD>((CGrannyModelInstance::ANIFPS_MAX - CGrannyModelInstance::ANIFPS_MIN) * fLODFactor / m_fLODDistance + CGrannyModelInstance::ANIFPS_MIN);
		else
		...
		BYTE bLODLevel = static_cast<BYTE>(fLODFactor / fLODStep);
		...
		if (m_fLODDistance <= 5000.0f)
		{
			...
			bLODLevel = static_cast<BYTE>(m_que_pkModelInst.size() - std::min<std::size_t>(bLODLevel, m_que_pkModelInst.size()) - 1);
		}
...
void CGrannyLODController::UpdateTime(float fElapsedTime)
{
	...
	{
		static FILE* pFile = fopen("perf_lod_update.txt", "w");
		if (dwLastTime - dwTime1 > 3)
		{
			fprintf(pFile, "LOD.Total %d (Time %f)\n", dwLastTime - dwTime1, timeGetTime() / 1000.0f);
			fprintf(pFile, "LOD.UP %d\n", dwLastTime - dwTime1);
			fprintf(pFile, "-------------------------------- \n");
			fflush(pFile);
		}
		fflush(pFile);
