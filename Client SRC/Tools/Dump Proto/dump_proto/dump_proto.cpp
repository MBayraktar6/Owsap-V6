...
bool Set_Proto_Mob_Table(TMobTable* mobTable, cCsvTable& csvTable, std::map<int, const char*>& nameMap)
{
	...
	mobTable->bSungMaStr = atoi(csvTable.AsStringByIndex(col++));
	mobTable->bSungMaDex = atoi(csvTable.AsStringByIndex(col++));
	mobTable->bSungMaCon = atoi(csvTable.AsStringByIndex(col++));
	mobTable->bSungMaInt = atoi(csvTable.AsStringByIndex(col++));
	...
	mobTable->dwSungMaExp = atoi(csvTable.AsStringByIndex(col++));
	...
	for (int i = 0; i < MOB_ELEMENT_MAX_NUM; ++i)
		mobTable->cElements[i] = atoi(csvTable.AsStringByIndex(col++));

	mobTable->cResistDark = atoi(csvTable.AsStringByIndex(col++));
	mobTable->cResistIce = atoi(csvTable.AsStringByIndex(col++));
	mobTable->cResistEarth = atoi(csvTable.AsStringByIndex(col++));
	...
	col++; //mobTable->dwPolymorphItemVnum = atoi(csvTable.AsStringByIndex(col++));

	for (int i = 0; i < MOB_SKILL_MAX_NUM; ++i)
	{
		col++; //mobTable->Skills[i].bLevel = atoi(csvTable.AsStringByIndex(col++));
		col++; //mobTable->Skills[i].dwVnum = atoi(csvTable.AsStringByIndex(col++));
	}

	col++; //mobTable->bBerserkPoint = atoi(csvTable.AsStringByIndex(col++));
	col++; //mobTable->bStoneSkinPoint = atoi(csvTable.AsStringByIndex(col++));
	col++; //mobTable->bGodSpeedPoint = atoi(csvTable.AsStringByIndex(col++));
	col++; //mobTable->bDeathBlowPoint = atoi(csvTable.AsStringByIndex(col++));
	col++; //mobTable->bRevivePoint = atoi(csvTable.AsStringByIndex(col++));
	col++; //mobTable->bHealPoint = atoi(csvTable.AsStringByIndex(col++));
	col++; //mobTable->bRAttSpeedPoint = atoi(csvTable.AsStringByIndex(col++));
	col++; //mobTable->bRCastSpeedPoint = atoi(csvTable.AsStringByIndex(col++));
	col++; //mobTable->bRHPRegenPoint = atoi(csvTable.AsStringByIndex(col++));
...
bool BuildMobTable()
{
	...
	*/
	...
	{
		fprintf(stderr, "mob_names.txt Failed to read file.\n"); // 파일을 읽어오지 못했습니다
		isNameFile = false;
	}
	else
	{
		...
		while (nameData.Next())
		{
			if (nameData.ColCount() < 2) continue;
	...
	{
		fprintf(stderr, "mob_proto_test.txt Failed to read file.\n"); // 파일을 읽어오지 못했습니다.
		// return false;
	}
	else
	{
		...
		{
			if (test_data.ColCount() < 2) continue;
	...
	{
		fprintf(stderr, "mob_proto.txt Failed to read file.\n"); // 파일을 읽어오지 못했습니다.
		return false;
	}
	...
	while (data.Next())
	{
		...
		else
		{
			...
			mob_table->bScale = tempTable->bScale;
			...
			mob_table->bSungMaStr = tempTable->bSungMaStr;
			mob_table->bSungMaDex = tempTable->bSungMaDex;
			mob_table->bSungMaCon = tempTable->bSungMaCon;
			mob_table->bSungMaInt = tempTable->bSungMaInt;
			...
			mob_table->dwSungMaExp = tempTable->dwSungMaExp;
			...
			for (int i = 0; i < MOB_ELEMENT_MAX_NUM; ++i)
				mob_table->cElements[i] = tempTable->cElements[i];
			mob_table->cResistDark = tempTable->cResistDark;
			mob_table->cResistIce = tempTable->cResistIce;
			mob_table->cResistEarth = tempTable->cResistEarth;
...
void SaveMobProto()
{
	...
	DWORD dwFourCC = MAKEFOURCC('M', 'M', 'P', 'T');
	fwrite(&dwFourCC, sizeof(DWORD), 1, fp);
...
bool BuildItemTable()
{
	...
	*/
	...
	{
		fprintf(stderr, "item_names.txt Failed to read file.\n"); // 파일을 읽어오지 못했습니다.
		isNameFile = false;
	}
	else
	{
	...
	{
		fprintf(stderr, "item_proto_test.txt Failed to read file.\n"); // 파일을 읽어오지 못했습니다.
		// return false;
	}
	else
	{
		...
		while (test_data.Next())
		{
			...
			{
				fprintf(stderr, "Item prototype table setting failed.\n"); // 몹 프로토 테이블 셋팅 실패.
			}
	...
	{
		fprintf(stderr, "item_proto.txt Failed to read file.\n"); // 파일을 읽어오지 못했습니다.
		return false;
	}
	data.Next(); //맨 윗줄 제외 (아이템 칼럼을 설명하는 부분)
	...
	while (data.Next())
	{
		...
		else
		{
			...
			item_table->ullAntiFlags = tempTable->ullAntiFlags;
			...
			item_table->dw67AttrMaterial = tempTable->dw67AttrMaterial;
			...
			for (i = 0; i < ITEM_APPLY_MAX_NUM; ++i)
			{
				item_table->aApplies[i].wType = tempTable->aApplies[i].wType;
		...
		fprintf(stdout, "ITEM #%-5u %-24s %-24s VAL: %ld %ld %ld %ld %ld %ld WEAR %u ANTI %llu IMMUNE %u REFINE %u\n",
			item_table->dwVnum,
			item_table->szName,
	...
	else
	{
		...
		while (test_data.Next()) // 테스트 데이터 각각을 훑어나가며,새로운 것을 추가한다.
		{
			...
			fprintf(stdout, "[NEW] ITEM #%-5u %-24s %-24s VAL: %ld %ld %ld %ld %ld %ld WEAR %u ANTI %llu IMMUNE %u REFINE %u\n",
				item_table->dwVnum,
				item_table->szName,
				item_table->szLocaleName,
				item_table->alValues[0],
